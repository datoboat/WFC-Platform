...
contract WfcRegistry is IWfcRegistry {
     ...
     */
    function addLogicalContract(string calldata _contractName, address payable _contractAddress) external onlyOwnerDao notZeroAddress(_contractAddress) {
     ...
     */
    function addStorageContract(string calldata _contractName, address payable _contractAddress) external onlyOwnerDao notZeroAddress(_contractAddress) {
        bytes32 contractName = _contractName.convert();
     ...
     */
    function updateLogicalContract(string calldata _contractName, address payable _newContractAddress) external onlyOwnerDao notZeroAddress(_newContractAddress) {
        bytes32 contractName = _contractName.convert();
     ...
     */
    function updateStorageContract(string calldata _contractName, address payable _newContractAddress) external onlyOwnerDao notZeroAddress(_newContractAddress) {
        bytes32 contractName = _contractName.convert();
        address previousAddress = logicalContracts[contractName];
     ...
     */
    function _addContract(bytes32 _contractName, address payable _contractAddress, ContractType _contractType ) internal {
        if (_contractType == ContractType.LOGICAL) {
     ...
     */
    function _updateContract(bytes32 _contractName, address payable _newContractAddress, ContractType _contractType ) internal {
        if (_contractType == ContractType.LOGICAL) {
            require(logicalContracts[_contractName] != address(0), "Logical contract already added"); 
