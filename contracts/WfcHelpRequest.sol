...
contract SplitContracts is OwnableDao  {
    ...
    constructor(Donation _donationContract, Reserve _reserveContract, ServiceLine _serviceLineContract, AllLines _allLinesContract, Team _teamContract, address _helpRequest) 
        internal 
...
contract WfcPlatformDonations is SupportedTokens, SupportedLines, DonationSplit, SplitContracts{
	...
	modifier onlyHelpRequest() {
	   require(msg.sender == helpRequest, 'sender is not helpRequest contract');
	   _;
	}
	
	function createFundraising(uint256 _line, address _organization, uint256 _softcap, uint256 _hardcap, address _currency, uint256 _timeout, address _destination, string calldata _metadata) external onlyHelpRequest {
	    uint256 propNumber = fundraisings[_line].push(FundraisingApproved(_organization, _softcap, _hardcap, 0, reserveSplit, serviceLineSplit, allLinesSplit, teamSplit, _currency, now + _timeout, _destination, false, _metadata));
        emit FundraisingCreatedLog(_line, propNumber - 1, _organization, _softcap, _hardcap, reserveSplit, serviceLineSplit, allLinesSplit, teamSplit, _currency, now + _timeout , _destination, _metadata);
	}
...
contract WfcPlatformLiquidate is WfcPlatformDonations {
    ...
    constructor(uint256 _warrantyTimeout, Donation _donationContract, Reserve _reserveContract, ServiceLine _serviceLineContract, AllLines _allLinesContract, address _helpRequest, Team _teamContract, uint256[4] memory _split, uint256 _numberLines, address[] memory _tokensAddress, address _ownerDao) 
        public
...
contract WfcPlatformRegistrations {
    ...
    function requireDaoInvestigation(string calldata _dataUrl, address _organizationAddress) external {
        OrganziationRef storage organziationRef = organizations[_organizationAddress];
        require(organziationRef.status == StatusRegistration.APPROVED, 'Organization not approved');
        ...
        organziationRef.dataUrlBlockRequest = _dataUrl;
    }
    
    function voteDaoBlock(address _organizationAddress, bool isBlocked) external onlyOwnerDao() {
        OrganziationRef storage organziationRef = organizations[_organizationAddress];
        require(organziationRef.status == StatusRegistration.APPROVED, 'Organization not approved');
        ...
        if (isBlocked) {
            ...
            organziationRef.dataUrlBlockRequest = '';
        }
    }
...
contract WfcPlatformProposals is WfcPlatformRegistrations {
	...
	constructor(WfcManagment _wfcManagment) public WfcPlatformRegistrations(_wfcManagment) {
	}
    
    
    function SubmitFundraising(string calldata _line, uint256 _softcap, uint256 _hardcap, address _tokenAddress, uint256 _timeout, address _destination, string calldata _metadata) external {
    ...
    function ApproveFundraising(uint256 _fundraisingId) external onlyOwnerDao() {
        ...
        wfcManagment.createFundraising(proposal.line, proposal.organization, proposal.softcap, proposal.hardcap, proposal.currency, proposal.timeout, proposal.destination, proposal.metadata);
    }
